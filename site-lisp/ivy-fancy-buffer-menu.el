...
(defvar ivy--buffer-info-fns
  (list
   ...
   (ivy--buffer-make-info-fn
    'host
    (with-current-buffer buffer_object
      (when (file-remote-p default-directory)
        (let* ((parts (tramp-dissect-file-name default-directory))
               (user (tramp-file-name-user parts))
               (host (tramp-file-name-host parts)))
          (concat
           (if (and user (> (length user) 0)) (concat user "@") "")
           (if (and host (> (length host) 0)) (car (split-string host "\\.")))
           )
          )))
    (let ((default-directory (file-name-directory (cdr (assq buffer ivy--virtual-buffers)))))
      (when (file-remote-p default-directory)
        (let* ((parts (tramp-dissect-file-name default-directory))
               (user (tramp-file-name-user parts))
               (host (tramp-file-name-host parts)))
